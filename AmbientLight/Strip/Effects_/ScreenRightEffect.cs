...
{
	class ScreenRightEffect : Effect
	{
		...
		public ScreenRightEffect(Logger logger, VirtualStrip strip, int maxFPS, ColorManager foreColorManager, ColorManager backColorManager)
		{
			this.logger = new Logger(logger);
			this.logger.AddLevel("ScreenRightEffect");
		...
		protected override void Loop(int deltatime)
		{
			...
			for (int i = 0; i < strip.parts.Count; i++)
			{
				...
				for (int j = 0; j < leds.Length; j++)
				{
					...
					//x1 counted, x2 over
					leds[j].SetRGB(ParcelColor(screen.Width - parcelHeight, screen.Width, x1, x2));
				}
			}
		}
